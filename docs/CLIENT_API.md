...
| `is_new_this_year` | Menu items only. True when this dish is new to this festival — read off the "(New)" mark the blogs write into their menu copy, so it's their claim, not Disney's. Safe to badge and filter on; **it is not exhaustive**, since a genuinely new dish nobody parenthesised reads as false. Deliberately narrow: DFB also hangs a "NEW!" banner on returning dishes (the Belgian Waffle has worn it for years), and that one is ignored, so a false badge is much less likely than a missing one. Added after the first release — treat a missing value as false. Not related to `origin`, which is about how the row got here rather than when the dish did. |
